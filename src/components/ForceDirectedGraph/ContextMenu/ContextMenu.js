/**
 * Context menu from https://github.com/m-thalmann/contextmenujs
 * slightly adjusted for ES6
 */
...
function ContextMenu(menu, options) {
  ...
  window.addEventListener('resize', function() {
    if (
      ContextUtil.getProperty(options, 'close_on_resize', true)
    ) {
      self.hide()
    }
  })
    ...
    level.forEach(function(item) {
      ...
      if (typeof item.type === 'undefined') {
        ...
        if (typeof item.sub !== 'undefined') {
          if (
            ContextUtil.getProperty(options, 'sub_icon', '') !==
            ''
          ) {
  ...
  this.display = function(e, target) {
    ...
    if (
      ContextUtil.getProperty(options, 'close_on_click', true)
    ) {
